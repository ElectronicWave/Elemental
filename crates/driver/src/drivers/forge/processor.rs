    ...
    VL: VersionJsonInstanceLayout,
{
        ...
        if let Some(artifact) = &library.downloads.artifact {
            ...
            if seen.insert(path.clone()) {
                tasks.push(DownloadTask::new(
                    remote_resolver
                        .forge_artifact_url(artifact.url.as_str(), artifact.path.as_str())?,
                    path,
    ...
    let report = downloader
        .run_plan(DownloadPlan::named(
            "forge-install-profile".to_owned(),
            tasks,
        ))
        .await
        .context("download forge install profile libraries failed")?;
    ...
    if report.failed > 0 {
        let failures = report
            ...
            .collect::<Vec<String>>()
            .join("\n");
        bail!("forge install profile library download failed:\n{failures}");
    }
    ...
    VL: VersionJsonInstanceLayout,
{
    ...
    let context = ForgeProcessorContext::new(
    ...
    if !context.client_outputs_ready()? {
        bail!("forge client processors completed without producing the expected client artifacts");
    }
...
#[derive(Clone)]
struct ForgeProcessorContext<'a> {
    ...
    forge_state_directory: PathBuf,
    minecraft_jar_path: PathBuf,
}

impl<'a> ForgeProcessorContext<'a> {
    fn new(
        root_directory: PathBuf,
        forge_state_directory: PathBuf,
        libraries_directory: PathBuf,
    ...
    fn resolve_variable(&self, key: &str) -> Result<String> {
        match key {
            ...
            _ => self
                ...
                .with_context(|| format!("unknown forge processor variable: {key}")),
        }
    }
...
async fn run_processor(
    runtime: &Distribution,
    context: &ForgeProcessorContext<'_>,
    ...
    let output = Command::new(runtime.executable())
        ...
        .output()
        .await
        .with_context(|| format!("run forge processor failed: {}", processor.jar.as_str()))?;
    ...
    bail!(
        "forge processor failed: processor={}, java={}, main_class={}, classpath={}, args={:?}, exit_status={:?}, stdout=\n{}\nstderr=\n{}",
        processor.jar,
